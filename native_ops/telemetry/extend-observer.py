...
def main(p):
 ...
 with (AUDIT/'install.lock').open('a') as lock:
  ...
  assert all(after.get(k)==v for k,v in before.items()) and len(after)==4
  compile(new,'fixed-read','exec')
  if old==new:return {'status':'already_installed','account':'ops-observer','telemetry':True}
  assert hashlib.sha256(old.encode()).hexdigest()==p['previous_reader_sha256'] and len(before)==3
  archive=AUDIT/('telemetry-'+str(time.time_ns()));archive.mkdir(mode=0o700);shutil.copy2(HELPER,archive/'reader.before.py');shutil.copy2(AUDIT/'current.json',archive/'current.before.json')
  try:
   ...
   result={'status':'installed','account':'ops-observer','telemetry':True,'previous_collectors_preserved':True,'reader_sha256':p['reader_sha256']};(archive/'result.json').write_text(json.dumps(result));return result
