...
ASSETS=Path('/usr/local/libexec/ops-zulip-public-20260912-1')
STATE=Path('/var/lib/ops-zulip-public')
...
def recover():
    ...
    subprocess.run(['systemctl','disable','--now','ops-zulip-tunnel.service'],capture_output=True)
...
def main():
    ...
    try:
        ...
        write('/etc/systemd/system/ops-zulip-tunnel.service',(ASSETS/'ops-zulip-tunnel.service').read_text(),0o644)
        run(['systemctl','daemon-reload']);run(['systemctl','enable','--now','ops-zulip-tunnel.service'])
        run(['systemctl','is-active','--quiet','ops-zulip-tunnel.service'])
