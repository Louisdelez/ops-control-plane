...
RELEASE = 'ops-native-2026.09.12.3'
...
def main():
    ...
    wheel = ROOT/'artifacts/global-memory-2026-09-12/wheels/ops_broker-0.1.4-py3-none-any.whl'
    ...
    try:
        ...
        run('/usr/sbin/runuser', '-u', 'opsbroker', '--', str(venv/'bin/python'), '-I', '-c',
            'from ops_broker.service import BrokerService; assert callable(BrokerService.list_memory_missions)')
        ...
        assert status['checked_at'] >= time.time()-180 and status['failed'] == 0 and not status['bounded_windows']
        published.write_bytes((ROOT/'native_ops/releases/current.json').read_bytes())
        published.chmod(0o444)
        receipt = {'release': RELEASE, 'broker_package': '0.1.4', 'archive': str(archive),
