...
MANIFEST=ROOT/'artifacts/completion-final-2026-09-14/provider-audit-capability.json'
MISSION='ce011514-448a-4194-a989-86292c11660d'
FILES={
 'broker/deploy/helpers/provider-access':('/usr/local/libexec/ops-runbooks/provider-access',0o755),
 'broker/deploy/sudoers/ops-broker-provider-access':('/etc/sudoers.d/ops-broker-provider-access',0o440),
 'runbooks/local/provider-access.yaml':('/opt/ops-broker/runbooks/local/provider-access.yaml',0o644),
}
...
def validate_release(manifest_path=MANIFEST,root=ROOT):
    ...
    installer=root/'native_ops/deploy/install-provider-audit-capability.py'
...
def install():
    ...
    archive=Path('/var/lib/ops-native-archives')/('provider-access-'+str(time.time_ns()))
    ...
    run('/usr/sbin/visudo','-cf',str(candidates[Path('/etc/sudoers.d/ops-broker-provider-access')][0]))
...
if 'inventory.provider-access.v1' not in b:b.append('inventory.provider-access.v1')
RBACPolicy.from_data(p)
e=yaml.safe_load(Path('/etc/ops-broker/executables.yaml').read_text())
h=e['sudo']['allowed_helpers']
if '/usr/local/libexec/ops-runbooks/provider-access' not in h:h.append('/usr/local/libexec/ops-runbooks/provider-access')
...
'''
    ...
    try:
        ...
        check="from pathlib import Path;from ops_broker.runbooks import ExecutablePolicy,RunbookRegistry;r=RunbookRegistry.load(Path('/opt/ops-broker/runbooks'),ExecutablePolicy.load(Path('/etc/ops-broker/executables.yaml')));assert r.get('inventory.provider-access.v1').action_class=='A'"
