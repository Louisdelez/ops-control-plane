"""Install a fixed encrypted seed transfer capability; live use requires real Class C approval."""
from pathlib import Path
import hashlib,json,os,runpy,shutil,sqlite3,subprocess,time
ROOT=Path('/home/ops-user/ops-control-plane');RELEASE='ops-native-2026.09.11.17'
assert os.geteuid()==0
assert runpy.run_path(str(ROOT/'native_ops/release.py'))['verify'](ROOT)['release_id']==RELEASE
archive=Path('/var/lib/ops-native-archives')/('recovery-seed-copy-'+str(time.time_ns()));archive.mkdir(parents=True,mode=0o700)
...
files={
 Path('/usr/local/libexec/ops-runbooks/recovery-seed-copy'):(source/'helpers/recovery-seed-copy',0o755),
 Path('/usr/local/libexec/ops-runbooks/receive-encrypted-seed.py'):(source/'recovery/receive-encrypted-seed.py',0o644),
 Path('/etc/sudoers.d/ops-broker-recovery-seed'):(source/'sudoers/ops-broker-recovery-seed',0o440),
 Path('/opt/ops-broker/runbooks/local/recovery-seed-copy.yaml'):(ROOT/'runbooks/local/recovery-seed-copy.yaml',0o644),
}
assert json.loads(Path('/usr/local/share/ops-native/recovery-seed/manifest.json').read_text())==json.loads((ROOT/'config/recovery-seed-manifest.json').read_text())
run('/usr/sbin/visudo','-cf',str(source/'sudoers/ops-broker-recovery-seed'))
...
if 'backup.recovery-seed-copy.v1' not in books:books.append('backup.recovery-seed-copy.v1')
RBACPolicy.from_data(rbac)
ex=yaml.safe_load(Path('/etc/ops-broker/executables.yaml').read_text())
helpers=ex['sudo']['allowed_helpers']
if '/usr/local/libexec/ops-runbooks/recovery-seed-copy' not in helpers:helpers.append('/usr/local/libexec/ops-runbooks/recovery-seed-copy')
...
try:
 ...
 check="from pathlib import Path;from ops_broker.runbooks import ExecutablePolicy,RunbookRegistry;r=RunbookRegistry.load(Path('/opt/ops-broker/runbooks'),ExecutablePolicy.load(Path('/etc/ops-broker/executables.yaml')));assert r.get('backup.recovery-seed-copy.v1').action_class=='C'"
 ...
 result={'release':RELEASE,'status':'installed','capability':'backup.recovery-seed-copy.v1','class':'C','remote_changes':False,'remote_checks_executed':False,'archive':str(archive),'manifest_sha256':hashlib.sha256(published.read_bytes()).hexdigest(),'verified_at':int(time.time())}
