...
MANIFEST=ROOT/'artifacts/completion-final-2026-09-14/logstream-capability.json'
MISSION='ce011514-448a-4194-a989-86292c11660d'
FILES={
 'broker/deploy/helpers/enable-logstream':('/usr/local/libexec/ops-runbooks/enable-logstream',0o755),
 'broker/deploy/sudoers/ops-broker-enable-logstream':('/etc/sudoers.d/ops-broker-enable-logstream',0o440),
 'runbooks/local/enable-logstream.yaml':('/opt/ops-broker/runbooks/local/enable-logstream.yaml',0o644),
}
...
def validate_release(manifest_path=MANIFEST,root=ROOT):
    ...
    installer=root/'native_ops/deploy/install-logstream-capability.py'
...
def install():
    ...
    archive=Path('/var/lib/ops-native-archives')/('enable-logstream-'+str(time.time_ns()))
    ...
    run('/usr/sbin/visudo','-cf',str(candidates[Path('/etc/sudoers.d/ops-broker-enable-logstream')][0]))
...
if 'security.enable-logstream.v1' not in b:b.append('security.enable-logstream.v1')
RBACPolicy.from_data(p)
e=yaml.safe_load(Path('/etc/ops-broker/executables.yaml').read_text())
h=e['sudo']['allowed_helpers']
if '/usr/local/libexec/ops-runbooks/enable-logstream' not in h:h.append('/usr/local/libexec/ops-runbooks/enable-logstream')
...
'''
    ...
    try:
        ...
        check="from pathlib import Path;from ops_broker.runbooks import ExecutablePolicy,RunbookRegistry;r=RunbookRegistry.load(Path('/opt/ops-broker/runbooks'),ExecutablePolicy.load(Path('/etc/ops-broker/executables.yaml')));assert r.get('security.enable-logstream.v1').action_class=='C'"
