...
RELEASE='ops-native-2026.09.12.5'
...
def main():
    ...
    archive=Path('/var/lib/ops-native-archives')/('jina-defaults-'+str(time.time_ns()))
    ...
    def write(p,raw,uid=0,gid=0,mode=0o644):
        p=Path(p);tmp=p.with_name(p.name+'.jina-next')
    ...
    try:
        ...
        paths=['kv-infra-shared/data/llm/providers/'+p for p in ['jina']]
        for path in paths:
        ...
        rendered=render(before,'jina-embeddings-v5-text-small','jina-reranker-v3.5',migrate=True)
