...
ROOT=Path('/home/ops-user/ops-control-plane');RELEASE='ops-native-2026.09.11.7'
assert os.geteuid()==0
assert runpy.run_path(str(ROOT/'native_ops/release.py'))['verify'](ROOT)['release_id']==RELEASE
archive=Path('/var/lib/ops-native-archives')/('locate-prod-'+str(time.time_ns()));archive.mkdir(parents=True,mode=0o700)
...
files={Path('/usr/local/libexec/ops-runbooks')/name:(source/'helpers'/name,0o755) for name in ['locate-prod','locate-prod-worker']}
files[Path('/etc/systemd/system/ops-locate-prod.service')]=(source/'systemd/ops-locate-prod.service',0o644)
files[Path('/etc/sudoers.d/ops-broker-locate-prod')]=(source/'sudoers/ops-broker-locate-prod',0o440)
files[Path('/opt/ops-broker/runbooks/local/locate-prod.yaml')]=(ROOT/'runbooks/local/locate-prod.yaml',0o644)
run('/usr/sbin/visudo','-cf',str(source/'sudoers/ops-broker-locate-prod'))
...
if 'inventory.locate-prod.v1' not in books:books.append('inventory.locate-prod.v1')
RBACPolicy.from_data(rbac)
ex=yaml.safe_load(Path('/etc/ops-broker/executables.yaml').read_text())
helpers=ex['sudo']['allowed_helpers']
if '/usr/local/libexec/ops-runbooks/locate-prod' not in helpers:helpers.append('/usr/local/libexec/ops-runbooks/locate-prod')
...
try:
 ...
 check="from pathlib import Path;from ops_broker.runbooks import ExecutablePolicy,RunbookRegistry;r=RunbookRegistry.load(Path('/opt/ops-broker/runbooks'),ExecutablePolicy.load(Path('/etc/ops-broker/executables.yaml')));assert r.get('inventory.locate-prod.v1').action_class=='A'"
 ...
 result={'release':RELEASE,'status':'installed','capability':'inventory.locate-prod.v1','class':'A','remote_changes':False,'remote_checks_executed':False,'archive':str(archive),'manifest_sha256':hashlib.sha256(published.read_bytes()).hexdigest(),'verified_at':int(time.time())}
