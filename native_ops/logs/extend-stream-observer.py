...
def main(p):
 ...
 with (AUDIT/'install.lock').open('a') as lock:
  ...
  assert all(after.get(k)==v for k,v in before.items()) and len(after)==6
  compile(new,'fixed-read','exec')
  if old==new:return {'status':'already_installed','account':'ops-observer','logstream':True}
  assert hashlib.sha256(old.encode()).hexdigest()==p['previous_reader_sha256'] and len(before)==5
  archive=AUDIT/('logstream-'+str(time.time_ns()));archive.mkdir(mode=0o700);shutil.copy2(HELPER,archive/'reader.before.py');shutil.copy2(AUDIT/'current.json',archive/'current.before.json')
  try:
   ...
   state=json.loads((AUDIT/'current.json').read_text());state.update(reader_sha256=p['reader_sha256'],telemetry=True,logstream=True);(AUDIT/'current.json').write_text(json.dumps(state))
   result={'status':'installed','account':'ops-observer','logstream':True,'previous_collectors_preserved':True,'reader_sha256':p['reader_sha256']};(archive/'result.json').write_text(json.dumps(result));return result
