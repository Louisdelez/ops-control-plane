...
def collect_stream(p):
 ...
 if p['source']=='system':
  ...
  with os.fdopen(fd,'rb') as f:
   ...
   f.seek(offset);discarding=cursor.get('discarding',False) if prior_inode==st.st_ino and not gap else False;consumed=0
   if type(discarding) is not bool:raise ValueError('Position invalide')
   for _ in range(BATCH):
    pos=f.tell();raw=f.readline(2*1024*1024+1);consumed+=len(raw)
    if not raw:break
    identity='docker:'+identifier+':'+str(st.st_ino)+':'+str(pos)
    if discarding:
     discarding=not raw.endswith(b'\n')
     if consumed>=8*1024*1024:break
     continue
    if len(raw)>2*1024*1024:
     gap=True;discarding=not raw.endswith(b'\n')
     records.append(event(time.time(),'docker',name,'Entrée Docker supérieure à 2 Mio : contenu non archivé, position conservée.',identity,'warning'))
     ...
     try:
      ...
      records.append(event(t,'docker',name,message,identity,level))
     except (ValueError,KeyError,TypeError,AttributeError):
      gap=True;records.append(event(time.time(),'docker',name,'Entrée Docker invalide : contenu non archivé, position conservée.',identity,'warning'))
    if consumed>=8*1024*1024:break
   offset=f.tell();more=offset<os.fstat(f.fileno()).st_size
   new={'inode':st.st_ino,'offset':offset,'last_t':records[-1]['t'] if records else cursor.get('last_t',since),'discarding':discarding}
