...
const RESUME_DEPLOYMENT_SOURCE_SHA256: &str =
    "0169a3fdbd0628cebb93b904a2a82343013c7c8bca4d7bae317dff9e4c648c00";
const RESUME_DEPLOYMENT_RPM_SHA256: &str =
    "8707a51b5139572a013c272c74d8f51d6769cd324f5bc45600c558065211b6be";
const RESUME_DEPLOYMENT_HERMES_SHA256: &str =
    "76b99a8be9b77d66833c3cfe2b35c6d6f6a58e4ff9637ef8effcfc1f420ab35a";
const RESUME_DEPLOYMENT_HELPER_SHA256: &str =
    "082d9ad7b6f5b6cb5111b66bf63f039237155523ac0ec0521c35f89de5558d23";
const RESUME_DEPLOYMENT_MANIFEST_SHA256: &str =
    ...
    matches!(
        ...
        DiskBootstrapState::Installed(installed)
            if installed.same_artifacts(&identity.release)
                || resume_patch_installed_proof_matches(&installed, identity)
    )
}

fn resume_patch_installed_proof_matches(
    installed: &ReleaseIdentity,
    current: &OperationIdentity,
) -> bool {
    current.mode == OperationMode::Corrective
        && current.recovery_from_release_id.as_deref() == Some(CORRECTIVE_PREDECESSOR_RELEASE_ID)
        && current.release.release_id == CORRECTIVE_TARGET_RELEASE_ID
        && current.release.source_sha256 == RESUME_DEPLOYMENT_SOURCE_SHA256
        && current.release.hermes_source_archive_sha256 == RESUME_DEPLOYMENT_HERMES_SHA256
        && current.release.helper_sha256 != RESUME_DEPLOYMENT_HELPER_SHA256
        && current.release.manifest_sha256 != RESUME_DEPLOYMENT_MANIFEST_SHA256
        && current.release.rpm_sha256 != RESUME_DEPLOYMENT_RPM_SHA256
        && installed.release_id == CORRECTIVE_TARGET_RELEASE_ID
        && installed.source_sha256 == RESUME_DEPLOYMENT_SOURCE_SHA256
        && installed.rpm_sha256 == RESUME_DEPLOYMENT_RPM_SHA256
        && installed.hermes_source_archive_sha256 == RESUME_DEPLOYMENT_HERMES_SHA256
        && installed.helper_sha256 == RESUME_DEPLOYMENT_HELPER_SHA256
        && installed.manifest_sha256 == RESUME_DEPLOYMENT_MANIFEST_SHA256
}
...
mod tests {
    ...
    #[test]
    fn resume_patch_accepts_only_the_exact_d_installed_proof() {
        let installed = ReleaseIdentity {
            release_id: CORRECTIVE_TARGET_RELEASE_ID.to_owned(),
            helper_sha256: RESUME_DEPLOYMENT_HELPER_SHA256.to_owned(),
            manifest_sha256: RESUME_DEPLOYMENT_MANIFEST_SHA256.to_owned(),
            source_sha256: RESUME_DEPLOYMENT_SOURCE_SHA256.to_owned(),
            rpm_sha256: RESUME_DEPLOYMENT_RPM_SHA256.to_owned(),
            hermes_source_archive_sha256: RESUME_DEPLOYMENT_HERMES_SHA256.to_owned(),
            confirmation_digest_suffix: "ignored-by-lineage-check".to_owned(),
        };
        let current = OperationIdentity {
            mode: OperationMode::Corrective,
            release: ReleaseIdentity {
                release_id: CORRECTIVE_TARGET_RELEASE_ID.to_owned(),
                helper_sha256: "a".repeat(64),
            ...
            recovery_from_release_id: Some(CORRECTIVE_PREDECESSOR_RELEASE_ID.to_owned()),
            rollback_finalize_resume: false,
        };

        assert!(resume_patch_installed_proof_matches(&installed, &current));
        let mut wrong = installed.clone();
        wrong.rpm_sha256 = "f".repeat(64);
        assert!(!resume_patch_installed_proof_matches(&wrong, &current));
        let mut wrong_mode = current;
        wrong_mode.mode = OperationMode::Fresh;
        assert!(!resume_patch_installed_proof_matches(
            &installed,
            &wrong_mode
        ));
    }
