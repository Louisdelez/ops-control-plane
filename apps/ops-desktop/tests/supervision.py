...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT.parents[1]/'artifacts/supervision-pro-2026-09-14';API='http://127.0.0.1:4467'
...
with tempfile.TemporaryDirectory(prefix='ops-telemetry-ui-') as temp:
 ...
 try:
  ...
  js("document.querySelector('#log-service').value='system|ops-telemetry.service';document.querySelector('#log-read').click();return true")
  for _ in range(100):
   if js('return !logBusy'):break
   time.sleep(.2)
  ...
  (OUT/'logs-native.png').write_bytes(base64.b64decode(call('GET',p+'/screenshot')))
  ...
  assert invoke('infrastructure_explore')['ok']
  for _ in range(40):
   handles=call('GET',p+'/window/handles')
   found=None
   for handle in handles:
    call('POST',p+'/window',{'handle':handle})
    if call('GET',p+'/url').startswith('http://127.0.0.1:3100/'):found=handle;break
   if found:break
   time.sleep(.25)
  assert found,'Grafana native window missing'
  for _ in range(60):
   if 'Ops · Infrastructure complète' in js('return document.body.textContent'):break
   time.sleep(.5)
  (OUT/'grafana-native.png').write_bytes(base64.b64decode(call('GET',p+'/screenshot')))
  assert 'Ops · Infrastructure complète' in js('return document.body.textContent'),js('return JSON.stringify({errors:window.__opsGrafanaErrors,title:document.title,loaded:window.__grafana_app_bundle_loaded,boot:!!window.grafanaBootData,chunks:window.webpackChunkgrafana?.map(x=>x[0]),resources:performance.getEntriesByType("resource").map(x=>({name:x.name,size:x.transferSize})).slice(-10)})')
  assert not invoke('infrastructure_snapshot')['ok']
  for _ in range(60):
   if 'En ligne' in js('return document.body.textContent'):break
   time.sleep(.5)
  assert 'En ligne' in js('return document.body.textContent')
  time.sleep(5)
  (OUT/'grafana-native.png').write_bytes(base64.b64decode(call('GET',p+'/screenshot')))
  report={'status':'passed','native_tauri':True,'hosts':len(live['value']['hosts']),'online':sum(h['status']=='online' for h in live['value']['hosts']),'topbar_tab':True,'detail_navigation':True,'theme':True,'period_selection':True,'compact_layout':True,'ipc_isolated':True,'four_live_charts':True,'storage_apple_bars':True,'historical_snapshot_verified':True,'grafana_native_isolated':True,'logs_read_search_archive_export':True,'retention_permanent':live['value'].get('retention_policy')=='permanent','stale_and_offline_states':True,'profile':'isolated','tested_at':time.time()};(OUT/'ui-test.json').write_text(json.dumps(report,indent=2));print(json.dumps(report))
 finally:
  if session:
