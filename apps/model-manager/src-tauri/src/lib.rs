...
/// Reuse Atlas commands in the Ops desktop host, without starting a second app.
/// The host must restrict callers and capabilities to its local Atlas webview.
pub fn handle_invoke(invoke: tauri::ipc::Invoke) -> bool {
...
pub fn initialize_embedded(app: &AppHandle) {
    use tauri::Manager;
    app.manage(onboarding::BootstrapManager::default());
}
