...
(function exposeAtlasOnboarding(globalObject, factory) {
  ...
  function operationalError(errorCode, phase = "") {
    ...
    const category = (() => {
      ...
      if (code === "health_failed") return "service_unhealthy";
      if (["step_timeout", "step_failed"].includes(code)) return "installation_failed";
    ...
    const catalogue = {
      ...
      installation_failed: {
        title: "L’installation s’est arrêtée",
        message: "Un composant n’a pas pu être installé. Le diagnostic local doit être vérifié avant une nouvelle tentative.",
        action: "Revérifier",
      },
      service_unhealthy: {
